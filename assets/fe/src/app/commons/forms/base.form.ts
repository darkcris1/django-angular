...
export class Form extends FormError {
  ...
  getErrMsgByKey(formControl: AbstractControl,key: string, options?: DisplayErrorOptions) {
    ...
    if (["minlength", "maxlength"].includes(key)) {
      ...
      keyValue = keyValue || formControl?.errors?.[key][key] 
    } else {
    ...
    const errMsg: {[key: string]: any} = {
      ...
      "min": "Must be greater than %keyValue& value",
      "max": "Must be less than %keyValue& value",
  ...
  displayError(key: string | AbstractControl, options?:DisplayErrorOptions): string[] | string { 
    ...
    return this.getErrMsgByKey(formControl,keys[0],options)
  }
